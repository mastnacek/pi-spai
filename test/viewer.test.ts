...
test("getStatusCounts strictly separates tasks from ideas and notes", () => {
      const index: SpaiIndex = {
            version: 1,
            lastUpdated: "2026-08-27 10:00:00",
            records: [
      ...
      assert.equal(counts.totalTasks, 3);
      assert.equal(counts.totalItems, 5);

      const ribbon = renderSpaiRibbon(counts, 20);
      assert.ok(ribbon.includes("[3/3]"));
      assert.ok(ribbon.includes("100%"));
});
